-- 삼육대학교 2026학년도 수시모집요강 39~40쪽의 학생부 교과/출결 산식을
-- 일괄 검증 결과에서도 재사용할 수 있도록 전형·모집단위별 게시 규칙으로 등록한다.
...
SELECT university.id,
    CONCAT('2026 ', variants.admission_type, ' ', variants.recruitment_unit, ' 교과성적'),
    2026, variants.admission_type, variants.recruitment_unit, 1,
    ...
    '2026학년도 삼육대학교 수시모집요강', '39-40',
    variants.interpretation_note,
    '2026 모집요강의 반영 교과, 이수단위 가중평균, 전형별 배점 및 성취도 환산표 반영',
    NULL,
    TRUE, 'PUBLISHED', 'guidebook-audit',
    '삼육대학교 공식 2026 수시모집요강 39~40쪽 검증', CURRENT_TIMESTAMP(6),
    'guidebook-audit', '원천 학생부 일괄 검증용 게시 규칙', CURRENT_TIMESTAMP(6),
...
JOIN (
    SELECT '학교장추천' admission_type, '일반학과(부)' recruitment_unit,
        ...
        '국어·영어·수학·탐구(사회·과학) 전 과목의 환산점수를 이수단위로 가중평균하여 1,000점 만점으로 환산합니다.' interpretation_note
    UNION ALL SELECT '학교장추천', '아트앤디자인학과', 'TOP_N_SUBJECTS', 2, 2.0000, 'GENERAL',
        '국어·영어·수학·탐구 중 성적이 높은 2개 교과영역 전 과목을 반영하여 교과 200점을 산출합니다.'
    UNION ALL SELECT '학교장추천', '체육학과', 'TOP_N_SUBJECTS', 2, 4.0000, 'GENERAL',
        '국어·영어·수학·탐구 중 성적이 높은 2개 교과영역 전 과목을 반영하여 교과 400점을 산출합니다.'
    UNION ALL SELECT '예체능인재', '체육학과', 'TOP_N_SUBJECTS', 2, 3.6000, 'GENERAL',
        '상위 2개 교과영역의 교과점수 360점과 출결점수 40점을 합산하여 학생부 400점을 산출합니다.'
    UNION ALL SELECT '특성화고교', '일반학과(부)', 'ALL_COURSES', 0, 10.0000, 'SPECIALIZED',
        '국어·영어·수학 전 과목의 환산점수를 이수단위로 가중평균하여 1,000점 만점으로 환산합니다.'
) variants
...
JOIN (
    ...
    UNION ALL SELECT 4, 96.5000
    UNION ALL SELECT 5, 95.0000
    UNION ALL SELECT 6, 92.0000
    UNION ALL SELECT 7, 85.0000
...
SELECT DISTINCT rule.university_id, 2026, rule.admission_type, TRUE, CURRENT_TIMESTAMP(6), CURRENT_TIMESTAMP(6)
FROM evaluation_rule rule
JOIN university university ON university.id = rule.university_id AND university.code = 'SY'
WHERE rule.admission_year = 2026 AND rule.status = 'PUBLISHED';
